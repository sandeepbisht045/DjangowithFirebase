...
# setting up firebase database to establish connection use your keys credentilas as I have removed it from below details
config = {"apiKey": "your api key", "authDomain": "krishworks-104a4.firebaseapp.com", "projectId": "krishworks-104a4", "storageBucket": "krishworks-104a4.appspot.com", "messagingSenderId": "your id", "appId": "1:your app id", "measurementId": "your id", "databaseURL": "https://krishworks-104a4-default-rtdb.firebaseio.com"
          }
...
def signup(request):
    if not request.session.get("user_id"):
        if request.method == "POST":
            ...
            database.child("images").child(local_id).set(
                {"image_link": "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png"})
            session_id = user["idToken"]
...
def profile(request):
    if request.session.get("user_id"):
        ...
        if request.method == "POST":
            ...
            l_id = auth.get_account_info(request.session.get("user_id"))[
                "users"][0]["localId"]
            email = auth.get_account_info(request.session.get("user_id"))[
                "users"][0]["email"]
            data = {"dob": dob, "address": address}
            database.child("users").child(l_id).child(
                "details").child("username").set(data)
            data_fetch = database.child("users").child(l_id).child(
                "details").child("username").shallow().get().val()
            list = []
            data_final = []
            for i in data_fetch:
                list.append(i)
            for i in list:
                data_filter = database.child("users").child(l_id).child(
                    "details").child("username").child(i).get().val()
            ...
            try:
                ...
                data_fetch = database.child("users").child(l_id).child(
                    "details").child("username").shallow().get().val()
                username_fetch = database.child(
                    "names").child(l_id).get().val()
                link = retrieve_image_link(l_id)
                list = []
                data_final = []
                for i in data_fetch:
